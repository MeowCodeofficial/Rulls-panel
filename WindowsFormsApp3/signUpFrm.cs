...
namespace WindowsFormsApp3
{
    public partial class signUpFrm : Form
    {
        ...
        public signUpFrm(UserTbl user)
        {
            ...
            txtConPass.Text = user.Password;
            txtPassword.Text = user.Password;
            numAge.Value = user.Age;
            var city = Context.CityTbls.Where(x => x.CityID == user.City_ID_FK);
            var state = Context.StateTbls.Where(x => x.StateID == city.FirstOrDefault().State_ID_FK);
            var country = Context.CountryTbls.Where(x => x.CountryID == state.FirstOrDefault().Country_ID_FK);
            var job = Context.JobTbls.Where(x => x.JobID == user.Job_ID_FK);
            comboCity.SelectedValue = city;
            comboState.SelectedValue = state;
            comboCountry.SelectedValue = country;
            comboJob.SelectedValue = job;
        }
